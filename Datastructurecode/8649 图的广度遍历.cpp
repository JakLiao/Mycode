...
int creat(Queue &L)
{
    L.front=(int*)malloc(MAX_VERTEX_NUM* sizeof(int));
	if(!L.front) return 0;
	L.rear=L.front;
	L.queuesize=MAX_VERTEX_NUM;
	return 1;
}

int getin(Queue &L,int e)
{
        ...
        if(visited[v]==0)
        {
          ...
          while(!QueueEmpty(L))
          {
            ...
            for(w=FirstAdjVex(G,G.vertices[v].data);w>=0;w=NextAdjVex(G,G.vertices[v].data,G.vertices[w].data))
               if(!visited[w])
               {
                 visited[w]=1;print(G.vertices[w].data);
                 getin(L,w);
               }
