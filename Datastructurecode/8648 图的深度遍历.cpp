...
{
    int w;/* 从第v个顶点出发递归地深度优先遍历图G。算法7.5 */
    visited[v]=1;/* 设置访问标志为TRUE(已访问) */
    print(G.vertices[v].data);/* 访问第v个顶点 */
    for(w=FirstAdjVex(G,G.vertices[v].data);w>0;w=NextAdjVex(G,G.vertices[v].data,G.vertices[w].data))/* 对v的尚未访问的邻接点w递归调用DFS */
        if(visited[w]==0) DFS(G,w);
}
void DFSTraverse(ALGraph G,void(*Visit)(char*))
{
    int v;/* 对图G作深度优先遍历。算法7.4 */
    VisitFunc=Visit;/* 使用全局变量VisitFunc,使DFS不必设函数指针参数 */
    for(v=0;v<G.vexnum;v++) {visited[v]=0;}/* 访问标志数组初始化 */
    for(v=0;v<G.vexnum;v++)
    {if(visited[v]==0) DFS(G,v);}/* 对尚未访问的顶点调用DFS */
	printf("\n");
}
